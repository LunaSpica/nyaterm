...
impl NyaTermApp {
    ...
    pub(in crate::features) fn cancel_transfer_editor_close_confirm(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::features) fn cancel_transfer_editor_reload_confirm(&mut self, cx: &mut Context<Self>) {
        let Some(state) = self.transfer_editor.as_mut() else {
            cx.notify();
            return;
        };
