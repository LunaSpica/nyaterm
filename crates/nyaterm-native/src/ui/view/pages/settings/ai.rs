...
impl NyaTermApp {
    ...
    pub(in crate::ui::view) fn ai_models_settings_section(
        ...
        let credential_rows = self
            ...
            .iter()
            .cloned()
            .fold(div().flex().flex_col().gap_2(), |rows, credential| {
                ...
                let is_expanded =
                    expanded_credential_id.as_deref() == Some(credential.id.as_str());
                let active_field = credential_edit.as_ref().and_then(|(id, field)| {
                    (id == &credential.id).then_some(*field)
                });
                                        ...
                                        .child(
                                            div()
                                                ...
                                                .child(format!(
                                                    ...
                                                    if is_builtin { " · built-in" } else { " · custom" }
                                                )),
                                        ),
                                )
                                .child(settings_switch(
