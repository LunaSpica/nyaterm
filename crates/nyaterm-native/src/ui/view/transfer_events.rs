            ...
            match event.event {
                ...
                TransferJobEvent::ExternalModified {
                    ...
                    if self.transfer_external_always_uploads.contains(&watch_key) {
                        external_sync_to_start =
                            Some((job.id.clone(), remote_path.clone(), local_path.clone()));
                    } else {
                        self.transfer_external_sync_prompt =
                            Some(TransferExternalSyncPromptState {
                                job_id: job.id.clone(),
                ...
                TransferJobEvent::Finished(Ok(TransferJobOutput::EditorSaved {
                        ...
                        && state.remote_path == remote_path
                    {
                        ...
                        match result {
                            nyaterm_session::SftpWriteTextResult::Saved { modified_at, size } => {
                            ...
                            nyaterm_session::SftpWriteTextResult::Conflict {
                                modified_at,
                                size,
                            } => {
                                state.base_size = Some(size);
                                ...
                                state.conflict = true;
                                state.close_after_save = false;
                                state.error = Some("Remote file changed before save.".to_string());
                                self.terminal_status =
