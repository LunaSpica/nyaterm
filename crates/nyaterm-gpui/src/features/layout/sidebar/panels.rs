...
impl NyaTermApp {
    pub(in crate::features) fn left_connections_panel(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    pub(in crate::features) fn left_network_panel(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
        let mut rows = div().flex().flex_col().gap_2();
    ...
    pub(in crate::features) fn left_transfers_panel(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
    ...
    pub(in crate::features) fn left_settings_panel(&mut self, _cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
        div()
            .flex()
            .flex_col()
