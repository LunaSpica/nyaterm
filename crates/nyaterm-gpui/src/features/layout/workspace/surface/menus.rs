...
impl NyaTermApp {
    pub(in crate::features) fn render_open_tabs_menu(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    pub(in crate::features) fn render_new_session_menu(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
