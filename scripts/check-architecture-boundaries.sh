...
check_exact_struct_fields \
  ...
  stores store_ui store_blocking runtime connection_state text_inputs selects commands remote_ops remote_desktop security settings ai terminal \
...
check_no_multiline_matches \
  "AI panel child must remain private to AiFeatureState" \
  'struct[[:space:]]+AiFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+panel[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/ai/state.rs
check_no_matches \
  "AI panel implementation type must remain private" \
  'pub([[:space:]]|\([^)]*\))[[:space:]]+struct[[:space:]]+AiPanelState' \
  crates/nyaterm-desktop/src/features/ai/state.rs
check_no_multiline_matches \
...
check_no_multiline_matches \
  ...
  crates/nyaterm-desktop/src/features
check_no_multiline_matches \
  "AI chat and agent children must remain private to AiFeatureState" \
  'struct[[:space:]]+AiFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+(chat|agent)[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/ai/state.rs
check_no_matches \
  "AI chat and agent implementation types must remain private" \
  'pub([[:space:]]|\([^)]*\))[[:space:]]+struct[[:space:]]+Ai(Chat|Agent)State' \
  crates/nyaterm-desktop/src/features/ai/state.rs
check_no_multiline_matches \
...
check_no_multiline_matches \
  ...
  crates/nyaterm-desktop/src/features
check_no_multiline_matches \
  "SettingsFeatureState store status must stay private to its owner implementation" \
  'struct[[:space:]]+SettingsFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+store_status[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/settings/state.rs
check_no_matches \
...
check_no_matches \
  ...
  crates/nyaterm-desktop/src/features/settings/mod.rs
check_no_multiline_matches \
  "settings compatibility children must use SettingsFeatureState methods" \
...
check_no_multiline_matches \
  ...
  crates/nyaterm-desktop/src/features
check_no_multiline_matches \
  "TransferFeatureState external_sync child must remain private" \
  'struct[[:space:]]+TransferFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+external_sync[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/transfers/state.rs
check_no_matches \
  "TransferExternalSyncState must remain private" \
  'pub([[:space:]]|\([^)]*\))[[:space:]]+struct[[:space:]]+TransferExternalSyncState' \
  crates/nyaterm-desktop/src/features/transfers/state.rs
check_no_multiline_matches \
  "TransferExternalSyncState fields must remain private" \
  'struct[[:space:]]+TransferExternalSyncState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+(prompts|windows|window_open_pending|always_uploads|focus)[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/transfers/state.rs
check_no_multiline_matches \
...
check_no_matches \
  ...
  crates/nyaterm-desktop/src/features/app_state/mod.rs
check_no_multiline_matches \
  "CommandFeatureState must not expose catalog, quick-command UI, history or persistence runtime" \
  'struct[[:space:]]+CommandFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+(catalog|quick|history|runtime)[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/commands/state.rs
check_no_multiline_matches \
  "QuickCommandFeatureState must not expose writable child states" \
  'struct[[:space:]]+QuickCommandFeatureState[[:space:]]*\{[^}]*pub([[:space:]]|\([^)]*\))[[:space:]]+(list|editor|dialogs|import|ai)[[:space:]]*:' \
  crates/nyaterm-desktop/src/features/commands/state.rs
check_no_multiline_matches \
  "command catalog, quick-command UI, history and persistence access must use CommandFeatureState methods" \
  '(self|this|app)[[:space:]]*\.[[:space:]]*commands[[:space:]]*\.[[:space:]]*(catalog|quick|history|runtime)([[:space:]]*\.|[[:space:]]*=)' \
  crates/nyaterm-desktop/src/features
