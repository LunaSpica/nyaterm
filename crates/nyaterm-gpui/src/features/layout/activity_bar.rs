...
impl NyaTermApp {
    ...
    pub(in crate::features) fn recording_activity_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
    ...
    pub(in crate::features) fn lock_activity_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
        div()
