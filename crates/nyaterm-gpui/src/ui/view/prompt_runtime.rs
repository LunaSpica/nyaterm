...
impl NyaTermApp {
    pub(in crate::ui::view) fn resolve_host_key_prompt(
        &mut self,
        request_id: String,
    ...
    pub(in crate::ui::view) fn resolve_duplicate_prompt(
        &mut self,
        request_id: String,
    ...
    pub(in crate::ui::view) fn submit_credential_prompt(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::ui::view) fn cancel_credential_prompt(&mut self, cx: &mut Context<Self>) {
        let Some(state) = self.active_credential_prompt.take() else {
            return;
        };
        let host = credential_prompt_target(&state.prompt);
    ...
    pub(in crate::ui::view) fn handle_credential_key_down(
        &mut self,
        event: &KeyDownEvent,
        cx: &mut Context<Self>,
    ) {
...
pub(in crate::ui::view) fn uuid_like_prompt_id(host_key: &SshHostKey) -> String {
...
pub(in crate::ui::view) fn credential_prompt_id(prompt: &SshCredentialPrompt) -> String {
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
...
pub(in crate::ui::view) fn sftp_duplicate_prompt_id(request: &SftpDuplicateRequest) -> String {
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
...
pub(in crate::ui::view) fn credential_prompt_target(prompt: &SshCredentialPrompt) -> String {
    format!(
        "{}@{}:{} (attempt {})",
