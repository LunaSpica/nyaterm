use super::{
    ...
    OverlaySnapshot, OverlayStore, RemoteOpsSnapshot, RemoteOpsStore, SessionSnapshot, SessionStore,
    SettingsSnapshot, SettingsStore, StartupRestoreStore, TransferSnapshot, TransferStore,
...
fn transfer_ai_sync_remote_stores_replace_snapshots() {
    ...
    assert!(CloudSyncStore::default().replace_snapshot(CloudSyncSnapshot {
        enabled: true,
        provider: "local".into(),
        conflict_active: false,
        last_status: "ok".into(),
    }));
    assert!(RemoteOpsStore::default().replace_snapshot(RemoteOpsSnapshot {
        process_count: 4,
