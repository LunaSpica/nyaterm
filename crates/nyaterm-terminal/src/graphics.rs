...
fn parse_iterm2_body(body: &[u8]) -> Option<GraphicsEvent> {
    ...
    let b64 = text[payload_sep + 1..].as_bytes();
        ...
        match mode {
            ...
            KittyDeleteMode::Newest => {
                let idx = self
                    ...
                    .enumerate()
                    .rev()
                    .find(|(_, p)| image_id.map_or(true, |id| p.kitty_id == Some(id)))
                    .map(|(i, _)| i);
                if let Some(idx) = idx {
            ...
            KittyDeleteMode::Placement => {
                ...
                self.placements.retain(|p| {
                    let image_ok = image_id.map_or(true, |id| p.kitty_id == Some(id));
...
fn peek_raster_size(data: &[u8]) -> Option<(u32, u32)> {
    ...
    if data.len() >= 10 && data[0] == 0xff && data[1] == 0xd8 {
        ...
        while i + 9 < data.len() {
            ...
            if matches!(marker, 0xc0 | 0xc1 | 0xc2) && i + 8 < data.len() {
