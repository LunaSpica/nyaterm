...
impl NyaTermApp {
    pub(in crate::ui::view) fn prompt_config_export(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::ui::view) fn prompt_portable_snapshot_export(&mut self, cx: &mut Context<Self>) {
        if self.config_path_prompt.is_some() {
            self.terminal_status = "config path picker is already open".to_string();
            cx.notify();
            return;
    ...
    pub(in crate::ui::view) fn prompt_encrypted_portable_snapshot_export(
        &mut self,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::ui::view) fn prompt_config_import(&mut self, cx: &mut Context<Self>) {
        if self.config_path_prompt.is_some() {
            self.terminal_status = "config path picker is already open".to_string();
            cx.notify();
            return;
    ...
    pub(in crate::ui::view) fn prompt_portable_snapshot_import(&mut self, cx: &mut Context<Self>) {
        if self.config_path_prompt.is_some() {
            self.terminal_status = "config path picker is already open".to_string();
            cx.notify();
            return;
    ...
    pub(in crate::ui::view) fn prompt_encrypted_portable_snapshot_import(
        &mut self,
        cx: &mut Context<Self>,
    ) {
        if self.active_session_id.is_some() || self.pending_session_name.is_some() {
    ...
    pub(in crate::ui::view) fn start_snapshot_password_prompt(
        &mut self,
    ...
    pub(in crate::ui::view) fn submit_snapshot_password_prompt(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::ui::view) fn cancel_snapshot_password_prompt(&mut self, cx: &mut Context<Self>) {
        let Some(state) = self.active_snapshot_password_prompt.take() else {
            return;
        };
    ...
    pub(in crate::ui::view) fn handle_snapshot_password_key_down(
        &mut self,
        event: &KeyDownEvent,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::ui::view) fn refresh_store_from_runtime(&mut self) {
