...
| AI settings/chat/history/discovery/agent/panel | Focused children in `NyaTermApp.ai`; panel private to `AiFeatureState` | Persisted settings plus transient UI and background lifecycle | Desktop consumers use read-only slices/queries and semantic transitions; settings draft groups, menu exclusion, confirmations, request/focus preparation, panel status/error ownership, detected-error throttling, picker clamping and Agent capture/reset enter through `AiFeatureState`. Persistence, terminal-context collection, GPUI focus/rendering and notification remain in adapters. |
