...
pub type CertificateVerifier = Arc<dyn Fn(&str, &x509_cert::Certificate) -> bool + Send + Sync>;
...
pub struct Config {
    ...
    pub(crate) certificate_verifier: Option<CertificateVerifier>,
...
impl fmt::Debug for Config {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        s.field("certificate_verifier", &self.certificate_verifier.as_ref().map(|_| "configured"));
        #[cfg(feature = "dvc-pipe-proxy")]
...
impl ConfigBuilder {
    ...
    /// Installs a certificate decision callback invoked after TLS exposes the
    /// server certificate and before CredSSP sends credentials.
    #[must_use]
    ...
    pub fn with_audio_playback(mut self, enabled: bool) -> Self {
        self.enable_audio_playback = Some(enabled);
        self
    }
