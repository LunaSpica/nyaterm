...
impl NyaTermApp {
    pub(in crate::features) fn security_secret_footer(&self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    pub(in crate::features) fn security_unlock_prompt(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        let palette = self.theme_palette();
    ...
    pub(in crate::features) fn security_tab_chip(&self, tab: SecurityAuthTab, cx: &mut Context<Self>) -> impl IntoElement {
