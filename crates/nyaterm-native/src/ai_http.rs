...
fn complete_openai_compatible_chat(
    ...
    request: &AiChatRequest,
    history: &[AiMessage],
    resolved_model: &nyaterm_domain::ResolvedAiModel,
) -> Result<AiChatCompletion, String> {
...
fn stream_gemini_chat(
    ...
    let model_name =
        nyaterm_domain::genai_model_name(&resolved_model.provider_kind, &resolved_model.model_name);
...
impl StreamToolCallBuffer {
    fn apply_delta(&mut self, delta: &nyaterm_domain::AiToolCallDelta) {
