    ...
    pub(in crate::features) fn cloud_sync_settings_section(
        ...
        div()
            ...
            .when(active_cloud_provider == "webdav", |this| {
                this.child(self.cloud_sync_webdav_provider_section(
                    webdav_password_value.clone(),
                    cx,
                ))
            })
            .when(active_cloud_provider == "s3", |this| {
                this.child(self.cloud_sync_s3_provider_section(
                    s3_access_key_value.clone(),
                    s3_secret_key_value.clone(),
                    s3_session_token_value.clone(),
                    cx,
                ))
            })
            .when(active_cloud_provider == "google_drive", |this| {
                this.child(self.cloud_sync_google_drive_provider_section(
                    google_drive_access_token_value.clone(),
                    google_drive_refresh_token_value.clone(),
                    google_drive_client_secret_value.clone(),
                    cx,
                ))
            })
            .when(active_cloud_provider == "onedrive", |this| {
                this.child(self.cloud_sync_onedrive_provider_section(
                    onedrive_access_token_value.clone(),
                    onedrive_refresh_token_value.clone(),
                    onedrive_client_secret_value.clone(),
                    cx,
                ))
            })
            .when(active_cloud_provider == "aliyun_drive", |this| {
                this.child(self.cloud_sync_aliyun_drive_provider_section(
                    aliyun_drive_access_token_value.clone(),
                    aliyun_drive_refresh_token_value.clone(),
                    aliyun_drive_client_secret_value.clone(),
                    cx,
                ))
            })
            .when(active_cloud_provider == "gitee_snippet", |this| {
                this.child(self.cloud_sync_gitee_provider_section(gitee_token_value.clone(), cx))
            })
            .when(active_cloud_provider == "github_gist", |this| {
                this.child(self.cloud_sync_github_provider_section(github_token_value.clone(), cx))
            })
            .child(settings_form_section(palette,
                Some("Sync actions"),
