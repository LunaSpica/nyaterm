...
impl NyaTermApp {
    pub(in crate::features) fn handle_smart_input_click(&mut self, event: &MouseUpEvent, cx: &mut Context<Self>) {
    ...
    pub(in crate::features) fn input_index_at_mouse(&self, position: Point<Pixels>) -> Option<usize> {
        if !self.can_use_smart_cursor_selection() {
            return None;
        }
